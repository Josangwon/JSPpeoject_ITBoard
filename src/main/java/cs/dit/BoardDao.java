...
public class BoardDao {
    ...
    private Connection getConnection() throws Exception {
        ...
        DataSource ds = (DataSource) initCtx.lookup("java:comp/env/jdbc/sangwon");
    ...
    public BoardDto getOne(String regdate) {
    	//3개의 테이블의 정보를 불러와서 최근 게시물 3개를 보여주는 sql문
        String sql = "SELECT subject, content, regdate, writer FROM sboard WHERE regdate=? union all "
        		+ "SELECT subject, content, regdate, writer FROM tboard WHERE regdate=? union all "
        		+ "SELECT subject, content, regdate, writer FROM qboard WHERE regdate=?";
        ...
        try (Connection con = getConnection();
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    dto = new BoardDto(subject, content, regDate, writer);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ArrayList<BoardDto> searchSboard(String search) {
    	//검색창에 검색한 단어를 포함한 데이터만 불러오기
    	String sql = "select snum, subject, writer from sboard where subject like '%"+search+"%' "
